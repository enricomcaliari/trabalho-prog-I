...
//definicao do tipo tEstatistica com suas variaveis
typedef struct {
  int jogadasPegouComida;
  int jogadasBateuParede;
} tEstatistica;
...
void ArquivoRanking(tJogo jogo, char caminho[1000]) {
  ...
  FILE *raFile = fopen(caminhofinal, "a");
  fclose(raFile);
}
...
void ArquivoEstatistica(tJogo jogo, char caminho[1000]) {
  ...
  FILE *eFile = fopen(caminhofinal, "a");
  fclose(eFile);
}
